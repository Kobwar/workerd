  ...
  KJ_IF_SOME(e, trace.eventInfo) {
    KJ_SWITCH_ONEOF(e) {
      ...
      KJ_CASE_ONEOF(tracedTrace, Trace::TraceEventInfo) {
        return kj::Maybe(jsg::alloc<TraceItem::TraceEventInfo>(trace, tracedTrace));
      }
...
kj::Maybe<TraceItem::EventInfo> TraceItem::getEvent(jsg::Lock& js) {
  return eventInfo.map([](auto& info) -> TraceItem::EventInfo {
    KJ_SWITCH_ONEOF(info) {
      ...
      KJ_CASE_ONEOF(info, jsg::Ref<TraceEventInfo>) { return info.addRef(); }
...
kj::Array<jsg::Ref<TraceItem::TraceEventInfo::TraceItem>> getTraceItemsFromEventInfo(
    const Trace::TraceEventInfo& eventInfo) {
  return KJ_MAP(t, eventInfo.traces) -> jsg::Ref<TraceItem::TraceEventInfo::TraceItem> {
    return jsg::alloc<TraceItem::TraceEventInfo::TraceItem>(t);
  };
}

TraceItem::TraceEventInfo::TraceEventInfo(const Trace& trace,
                                          const Trace::TraceEventInfo& eventInfo)
    : traces(getTraceItemsFromEventInfo(eventInfo)) {}

kj::Array<jsg::Ref<TraceItem::TraceEventInfo::TraceItem>> TraceItem::TraceEventInfo::getTraces() {
  return KJ_MAP(trace, traces) -> jsg::Ref<TraceEventInfo::TraceItem> {
    return trace.addRef();
  };
}

TraceItem::TraceEventInfo::TraceItem::TraceItem(const Trace::TraceEventInfo::TraceItem& traceItem)
    : scriptName(traceItem.scriptName.map([](auto& s) { return kj::str(s); })) {}

kj::Maybe<kj::StringPtr> TraceItem::TraceEventInfo::TraceItem::getScriptName() {
  return scriptName;
}
